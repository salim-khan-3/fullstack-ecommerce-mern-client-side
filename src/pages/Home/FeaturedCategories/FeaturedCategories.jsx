...
const FeaturedCategories = () => {
  const { categories, catLoading } = useContext(StoreContext);
  ...
  return (
    <>
      <style>{`
        ...
        .cat-card { transition: all 0.35s cubic-bezier(0.34, 1.56, 0.64, 1); }
        .cat-card:hover { transform: translateY(-6px) scale(1.03); }
        .cat-img { transition: transform 0.4s cubic-bezier(0.34, 1.56, 0.64, 1); }
        .cat-card:hover .cat-img { transform: scale(1.12) rotate(-3deg); }
        .skeleton-wave {
        ...
        .scroll-btn { transition: all 0.25s ease; }
        .scroll-btn:hover { transform: scale(1.1); box-shadow: 0 8px 25px rgba(0,0,0,0.15); }
        .title-line { width: 40px; height: 3px; border-radius: 2px; background: linear-gradient(90deg, #3b82f6, #8b5cf6); }
      `}</style>
        ...
        <div className="container mx-auto">
          ...
          {catLoading ? (
            ...
            <div className="relative">
                ...
                <div className="flex gap-3 md:gap-4">
                  {categories.map((cat, index) => {
                    ...
                    return (
                      ...
                      >
                        {/* ✅ Link — /category/:id এ navigate করবে */}
                        <Link to={`/category/${cat._id}`}>
                          <div className={`cat-card bg-gradient-to-b ${theme.bg} rounded-2xl border border-transparent ${theme.border} cursor-pointer overflow-hidden shadow-sm hover:shadow-xl`}>
              ...
              {/* Mobile arrows */}
              <button onClick={scrollPrev} className="md:hidden scroll-btn absolute -left-3 top-1/2 -translate-y-1/2 w-8 h-8 bg-white shadow-lg rounded-full flex items-center justify-center text-gray-600 border border-gray-100 z-10">
                <ChevronLeft size={16} />
              </button>
              <button onClick={scrollNext} className="md:hidden scroll-btn absolute -right-3 top-1/2 -translate-y-1/2 w-8 h-8 bg-white shadow-lg rounded-full flex items-center justify-center text-gray-600 border border-gray-100 z-10">
